...
const VERTEX_SHADER = `
  ...
  void main() {
    float depth = pow(smoothstep(0.02, 0.98, a_depth), 0.82);
    float travel = clamp(u_progress, 0.0, 1.0) * u_strength;
    vec2 focus = vec2(0.03, -0.28);
    float camera_push = 1.0 + travel * 0.22;
    float depth_relief = 1.0 + depth * travel * 0.35;
    float perspective = camera_push * depth_relief;
    vec2 position = focus + (a_position - focus) * perspective;

    // Near geometry advances while the sky and volcano barely recede.
    position.y += mix(-0.035, 0.22, depth) * travel;
...
const FRAGMENT_SHADER = `
  ...
  void main() {
    ...
    color.rgb *= 1.0 + (v_depth - 0.35) * v_travel * 0.025;
    gl_FragColor = color;
  }
`;
...
function createParallaxRenderer({
  ...
  const resize = () => {
    ...
    const zoom = canvasAspect < 0.8 ? 0.9 : 0.88;
    const strength =
      canvasAspect < 0.8 ? 0.9 : canvasAspect < 1.2 ? 0.96 : 1;
